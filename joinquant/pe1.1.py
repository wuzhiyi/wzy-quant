...
def initialize(context):
    # 初始化沪深300
    # 导入研究模块导出的47只龙头股在05年到09年底的PE历史数据
    a=read_file('47longtoustocks_PE_05_09.csv')    #打开文件
    g.operate_PEs = pickle.loads(a)
    g.stocks_ttl = ['000651.XSHE','601318.XSHG','000012.XSHE','002299.XSHE','600150.XSHG','600027.XSHG','600900.XSHG'
...
def before_trading_start(context):
    ...
    for i in range(0,length):
        ...
        if not current_data[g.stocks_ttl[i]].paused:
            hdict = attribute_history(g.stocks_ttl[i],g.longMA+1, '1d', ('close'),skip_paused=True,df = False)
            ...
            if len(temp_hist)>=g.longMA:
                ...
                signal=cal_signal(context,temp_hist,g.stocks_ttl[i])
    ...
    if length>0:
        ...
        for i in range(0,length):
            ...
            if not current_hold[g.stocks_hold[i]].paused:
                temp_hist= attribute_history(g.stocks_hold[i],g.longMA+1, '1d', ('close'),skip_paused=True,df=False)
                signal=cal_signal(context,temp_hist['close'],g.stocks_hold[i])
                if signal==-1:
...
#数字1:买入，-1卖出，0表示不变
def cal_signal(context,data_withHist,stock):
    ...
    downvalue = value_down(context,stock)
    signal=0
    if downvalue ==1 and MA_shortBeforeYesterday<MA_longBeforeYesterday and MA_shortYesterday>MA_longYesterday:
        signal=1
    elif downvalue ==-1 and MA_shortYesterday<MA_longYesterday:
        signal=-1
    return signal

# 判断某一只股票是否低估
def value_down(context,stock):
    year = context.current_dt.year
    month = context.current_dt.month
    day = context.current_dt.day
    end = datetime.date(year,month,day)
    begin = end - datetime.timedelta(days=g.days)
    d = begin
    Pe_ratio = []
    # 取出该只历史PE统计中的时间
    period = sorted(g.operate_PEs.keys())[-1*g.days:]
    for d in period:
        if not math.isnan(g.operate_PEs[d].ix[stock].values):
            Pe_ratio.append(g.operate_PEs[d].ix[stock].values)
    downvalue = 0
    if Pe_ratio[-1]<pd.Series(Pe_ratio[:-1]).quantile(q=g.down):
        downvalue = 1
    if Pe_ratio[-1]>pd.Series(Pe_ratio[:-1]).quantile(q=g.up):
        downvalue = -1
    return downvalue

# 取出当前日所有股票的PE存到历史数据(operate_PEs)里
def historyPEs(context):
    year = context.current_dt.year
    month = context.current_dt.month
    day = context.current_dt.day
    end = datetime.date(year,month,day)
    # 取出当前日的PE,返回的是dataframe，只能返回有值的股票
    q = query(valuation.code,valuation.pe_ratio).filter(valuation.code.in_(g.stocks_ttl))#不同报表只需修改valuation为相应报表名称。
    df = get_fundamentals(q, end)
    # 建立一个新的dataframe
    df1 = pd.DataFrame(index = df['code'].values)
    df1['pe_ratio']=df['pe_ratio'].values
    df2 = pd.DataFrame(index = g.stocks_ttl)
    df2['pe_ratio'] = 0
    #更新到历史数据里，当日没有值的为nan
    g.operate_PEs.update({end:(df1+df2)})
...
def handle_data(context,data):
    ...
    if num_canBuy == 0 and len(g.stocks_toBuy)>0:
        ...
        for i in range(0,min(len(g.stocks_toBuy),g.num_of_change)):
            order_target(stocks_holdreturnup.index[i],0, LimitOrderStyle(0.01))
        ...
        for i in range(0,num_canBuy):
            order_target_value(stocks_tobuyreturnup.index[len(stocks_tobuyreturnup)-i-1],cash_perShare,LimitOrderStyle(9999))
    ...
    # 如果坑没满
        ...
        if len(g.stocks_toBuy)<=num_canBuy:
            for i in range(0,len(g.stocks_toBuy)):
                order_target_value(g.stocks_toBuy[i],cash_perShare,LimitOrderStyle(9999))
